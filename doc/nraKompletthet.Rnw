...
<<'Kval.ind.kompletthet', results='asis', echo=FALSE>>=
...
RegData_forlop_1 <- merge(RegData_pre, RegData[RegData$ForlopsType1Num %in% 3, ], by.x = "ForlopsID",
                          by.y = "KobletForlopsID", all = T, suffixes = c('', '_oppf'))

RegData_forlop_1$beggetom <- is.na(RegData_forlop_1$StMarksTotalScore) & is.na(RegData_forlop_1$StMarksTotalScore_oppf)
