...
\title{Figurer og tabeller til årsrapport for NRA 2025}
\author{NRA}
...
<<'Oppfølginger', results='asis', echo=FALSE>>=

opdata <- RegData |> filter(ForlopsType1Num %in% 1:2) |>
  select(ForlopsID, PasientID, SenterKortNavn, Aar,
         ForlopsType1, ForlopsType1Num,
         ForlopsType2, ForlopsType2Num, OppflgRegStatus,
         HovedDato, Avdod, AvdodDato) |>
  mutate(tid_op_doed = difftime(AvdodDato, HovedDato, units = "days"))
oppfdata <- RegData |> filter(ForlopsType1Num %in% 3:4) |>
  select(KobletForlopsID, BasisRegStatus, ForlopsType1,
         ForlopsType1Num, OppflgRegStatus)

samlet <- merge(opdata,
                oppfdata |> filter(ForlopsType1Num == 3),
                by.x = "ForlopsID",
                by.y = "KobletForlopsID",
                all.x = TRUE, suffixes = c("", "_1aar")) |>
  merge(oppfdata |> filter(ForlopsType1Num == 4),
        by.x = "ForlopsID",
        by.y = "KobletForlopsID",
        all.x = TRUE, suffixes = c("", "_5aar")) |>
  filter(ForlopsType2Num == 2 | ForlopsType1Num == 1,
         Aar <= rap_aar-1) |>
  mutate(
    fulgtopp_1aar = ifelse(is.na(BasisRegStatus), 0, BasisRegStatus),
    fulgtopp_5aar = ifelse(
      is.na(BasisRegStatus_5aar), 0, BasisRegStatus_5aar)
  )




# table(samlet$fulgtopp_1aar, useNA = 'ifany')
# tmp <- samlet |> filter(fulgtopp_1aar == 0)

tmp <- samlet |>
  filter(Aar >= rap_aar-5) |>
  summarise(
    N = n(),
    fulgtopp = sum(BasisRegStatus == 1, na.rm = TRUE),
    .by = SenterKortNavn) |>
  janitor::adorn_totals()

# tmp <- samlet |> filter(ForlopsType2Num == 2 | ForlopsType1Num == 1,
#                        Aar <= 2024,
#                        Avdod == 1)
#
# table(samlet |> select(OppflgRegStatus, BasisRegStatus),
#       useNA = 'ifany')
#
# table(samlet |> filter(ForlopsType2Num == 2 | ForlopsType1Num == 1,
#                        Aar <= 2024) |>
#         select(OppflgRegStatus, BasisRegStatus),
#       useNA = 'ifany')
...
figfolder <- "C:/Users/kth200/regdata/nra/aarsrapp2025/indikatorer/"
...
aux <- merge(RegData[RegData$ForlopsType1Num %in% 1:2, !(names(RegData) %in% c("ICIQ_hyppighet", "LekkerUrinAldri", "LekkerUrinFoerToalett",
                                                                               "LekkerUrinHoster", "LekkerUrinSover", "LekkerUrinFysiskAktiv",
                                                                               "LekkerUrinEtterToalett", "LekkerUrinIngenGrunn",
                                                                               "LekkerUrinHeleTiden", "LekkerUrinUkjent"))],
             RegData[RegData$ForlopsType1Num %in% 3,
                     c("KobletForlopsID", "ICIQ_hyppighet",
                       "LekkerUrinAldri", "LekkerUrinFoerToalett",
                       "LekkerUrinHoster", "LekkerUrinSover",
                       "LekkerUrinFysiskAktiv", "LekkerUrinEtterToalett",
                       "LekkerUrinIngenGrunn", "LekkerUrinHeleTiden",
                       "LekkerUrinUkjent")],
             by.x = "ForlopsID", by.y = "KobletForlopsID")
nraFigAndeler(RegData=aux,
              valgtVar='lekker_urin_naar',
...
nraFigAndeler(RegData=aux,
              ...
              reshID=reshID, enhetsUtvalg=enhetsUtvalg,
              preprosess=preprosess, hentData=hentData,
              valgtShus = valgtShus, forlopstype1=1,
              forlopstype2=forlopstype2)
if (utformat!='pdf') {
  outfile <- paste0(substr(outfile, 1,nchar(outfile)-3), utformat)
  nraFigAndeler(RegData=aux,
...
nraFigAndeler(RegData=aux,
              ...
              reshID=reshID, enhetsUtvalg=enhetsUtvalg,
              preprosess=preprosess, hentData=hentData,
              valgtShus = valgtShus, forlopstype1=2,
              forlopstype2=forlopstype2)
if (utformat!='pdf') {
  outfile <- paste0(substr(outfile, 1,nchar(outfile)-3), utformat)
  nraFigAndeler(RegData=aux,
...
nraFigAndeler(RegData=RegData,
              valgtVar='Tilfredshet_1aar',
              datoFra=datoFra, datoTil=datoTil,
              minald=minald, maxald=maxald,
              erMann=erMann, outfile=outfile,
              reshID=reshID, enhetsUtvalg=enhetsUtvalg,
...
nraFigAndeler(RegData=RegData, valgtVar='Tilfredshet_1aar', datoFra=datoFra, datoTil=datoTil,
              minald=minald, maxald=maxald, erMann=erMann, outfile=outfile,
              reshID=reshID, enhetsUtvalg=enhetsUtvalg, preprosess=preprosess, hentData=hentData,
              valgtShus = valgtShus, forlopstype1=1, forlopstype2=forlopstype2)
if (utformat!='pdf') {
