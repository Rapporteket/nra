...
RegData <- read.table('I:/nra/alleVarNum2021-08-10 10-35-35.txt', header=TRUE, sep=";", encoding = 'UTF-8')
...
ForlopData <- read.table('I:/nra/ForlopsOversikt2021-08-10 10-35-35.txt', header=TRUE, sep=";", encoding = 'UTF-8')
...
stmarks_under9 <- RegData[which(RegData$StMarksTotalScore<=9 & RegData$ForlopsType1Num %in% 1:2 & RegData$Aar == (rap_aar-1)), ]
...
stmarks_under12<- RegData[which(RegData$StMarksTotalScore<=12 & RegData$ForlopsType1Num %in% 1:2 & RegData$Aar == (rap_aar-1)), ]
...
wex_under9 <- RegData[which(RegData$WexnerTotalScore<=9 & RegData$ForlopsType1Num %in% 1:2 & RegData$Aar == (rap_aar-1)), ]
...
wex_under12 <- RegData[which(RegData$WexnerTotalScore<=12 & RegData$ForlopsType1Num %in% 1:2 & RegData$Aar == (rap_aar-1)), ]
...
nraGjsnPrePost(RegData, valgtVar='Urinlekkasje', datoFra='2013-01-01', datoTil=paste0(rap_aar-4, '-12-31'),
               outfile = outfile, preprosess=preprosess, minald=minald, maxald=maxald,
               erMann=erMann, reshID=reshID, hentData=hentData, forlopstype1=2, forlopstype2=forlopstype2,
