...
aux <- RegData %>% dplyr::filter(PostopKomplikasjoner==1) %>%
  dplyr::group_by(Aar) %>%
  dplyr::summarise(
    Bloedning = sum(Bloedning),
    Saardehisens = sum(Saardehisens),
    Saarinfeksjon = sum(Saarinfeksjon),
    N = n()
  )

RegData$Bloedning
RegData$Saardehisens
RegData$Saarinfeksjon

table(RegData$Komplikasjon, useNA = 'ifany')
