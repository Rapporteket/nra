...
#
#
fordelingsfig_prepost_UI <- function(id, BrValg){
  ns <- shiny::NS(id)
...
fordelingsfig_prepost <- function(input, output, session, reshID, RegData, hvd_session){
  ...
  output$Figur1 <- renderPlot({
    nraBeregnFordelingPrePost(RegData = RegData, valgtVar = input$valgtVar,
  ...
  output$lastNedBilde <- downloadHandler(
    ...
    content = function(file){
      nra::nraBeregnFordelingPrePost(RegData = RegData, valgtVar = input$valgtVar,
                                     minald=as.numeric(input$alder[1]),
                                     maxald=as.numeric(input$alder[2]), datoFra = input$datovalg[1],
                                     datoTil = input$datovalg[2],
                                     valgtShus = if (!is.null(input$valgtShus)) {input$valgtShus} else {''},
