...
#'
#' @return Datadump av registerets tabeller
#' @export
#'
  ...
  shiny::sidebarLayout(
    sidebarPanel(
      ...
      dateRangeInput(inputId=ns("datovalg"),
                     label = "Dato fra og til", language = "nb",
    ...
    mainPanel(
      ...
      br(),
      h4('Her kan du laste ned forskjellige varianter av datadump for NRA.
      ...
      h4(tags$b('allevarnum_utflatet '), 'inneholder alle kliniske variabler
      i registeret og benytter tallkodene til kategoriske variabler.
         At tabellen er utflatet innebærer at oppfølginger er koblet til sine
         respective basisregistreringer slik at en linje utgjør et forløp.')
    )
  )
}
...
#' @export
#'
datadump_server <- function(id, reshID, userRole, hvd_session){
  moduleServer(
    id,
      ...
      output$lastNed_dump <- downloadHandler(
        ...
        content = function(file){
          if (
            ...
            tmpData <- read.table(
              paste0('I:/nra/', input$dumptype, '2020-09-08.txt'),
              header=TRUE, sep=";", encoding = 'UTF-8', stringsAsFactors = F)
          }
          dumpdata <- tmpData[which(
            as.Date(tmpData$HovedDato) >= input$datovalg[1] &
      ...
      shiny::observe({
        if (rapbase::isRapContext()) {
          ...
          shinyjs::onclick(
            ...
            rapbase::repLogger(
              session = hvd_session,
              msg = paste0("NRA: nedlasting datadump: ",
                           input$dumptype)
            )
          )
        }
