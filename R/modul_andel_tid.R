#' Modul for tidsfigur i NRA sin shiny-app på Rapporteket
#'
#' Kun til bruk i Shiny
#'
#' @inheritParams nraFigAndeler
#'
#' @return ui-del av tidsfigur
#' @export
#'
andeler_tid_ui <- function(id){
  ns <- shiny::NS(id)

  shiny::sidebarLayout(
    sidebarPanel(
      selectInput(
        inputId = ns("valgtVar"), label = "Velg variabel",
        choices =
          c("Tidligere konservativt behandlet" = "tidligereKonservativ",
            "Utført ultralyd" = "Ultralyd_utfort",
            "Mer enn 50% reduksjon i lekkasje under test" =
              "Indikator1_lekk_red50",
            "Bekreftet sårinfeksjon ved SNM" = "saarinfeksjon_snm")),
      dateRangeInput(
        inputId=ns("datovalg"), label = "Dato fra og til",
        max = Sys.Date(),
        start  = lubridate::floor_date(lubridate::today() - lubridate::years(2),
                                       unit = "year"),
        end = Sys.Date(), language = "nb", separator = " til "),
      selectInput(
        inputId = ns("enhetsUtvalg"), label = "Lag figur for",
      ...
      selectInput(
        ...
        choices = c('Mnd', 'Kvartal', 'Halvaar', 'Aar'),
        selected = 'Kvartal'),
      selectInput(
        inputId = ns("inkl_konf"), label = "Inkluder konfidensintervall",
        choices = c('Nei'=0, 'Ja'=1)),
      sliderInput(
        inputId=ns("alder"), label = "Alder", min = 0,
        max = 130, value = c(0, 130)),
      selectInput(
        inputId = ns("erMann"), label = "Kjønn",
        choices = c('Begge'=99, 'Kvinne'=0, 'Mann'=1)),
      selectInput(
        inputId = ns("forlopstype1"), label = "Velg operasjonstype",
        choices = c('Sfinkterplastikk'=1, 'SNM'=2),
        multiple = TRUE),
      uiOutput(outputId = ns('forlopstype2')),
      shiny::selectInput(
        inputId = ns("onestage"), label = "One stage",
        choices = c('--'=99, 'Ja'=1, 'Nei'=0), selected = 99),
      selectInput(
        inputId = ns("bildeformat"), label = "Velg bildeformat",
...
#' @inheritParams nraFigAndeler
#'
#' @return server-del av tidsfigur
#' @export
#'
andeler_tid_server <- function(id, reshID, RegData, hvd_session,
                               BrValg, userRole){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$Figur1 <- renderPlot({
        nra::nraFigAndelTid(
          ...
          outfile = '',
          tidsenhet = if (!is.null(input$tidsenhet)) {
            input$tidsenhet} else {99},
          inkl_konf = if (!is.null(input$inkl_konf)) {
            input$inkl_konf} else {99},
      ...
      output$Tabell_tid <- function() {
        ...
        if (input$enhetsUtvalg == 1) {
          ...
          names(Tabell_tid) <- c(
            'Tidsperiode', 'Antall', 'N', 'Andel (%)', 'KI_nedre',
            'KI_ovre', 'Antall', 'N', 'Andel (%)', 'KI_nedre', 'KI_ovre')
          Tabell_tid %>% knitr::kable(
            "html", digits = c(0,0,0,1,1,1,0,0,1,1,1)) %>%
            kableExtra::kable_styling("hover", full_width = F) %>%
            kableExtra::add_header_above(c(" ", "Din avdeling" = 5,
                                           "Landet forøvrig" = 5))
        } else {
          Tabell_tid <- dplyr::tibble(
            Tidsperiode = utdata$Tidtxt,
